$(function() {
    ...
    $('.table thead a').on('click', function(e) {
        $('.table').attr('data-orden', $(e.currentTarget).attr('data-value'));
        paginacion(e);
    });

    function eventoBorrar(e) {
        e.preventDefault();
        if (confirm('¿Estás seguro de borar este enlace?')) {
    ...
    function paginacion(e) {
        e.preventDefault();
        genericAjax('ajax/mostrarLinks', { pagina: $(e.currentTarget).attr('data-pagina'), orden: $('.table').attr('data-orden') }, 'get', function(json) {
            let tabla = '';
